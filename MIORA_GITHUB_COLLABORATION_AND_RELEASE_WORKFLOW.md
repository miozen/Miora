...
```text
feature/<topic> -- PR --> dev -- PR + release tag --> main
                                  |                    |
                                  |                    +-- GitHub Actions：测试、构建、推送 GHCR
                                  +-- GitHub Actions：CI 验证，不推送生产镜像
```
...
| 合并到 `main` | 是 | 是，可标记候选版本 | 生产候选 |
| 推送 `vX.Y.Z` 标签 | 是或复用已验证结果 | 是，固定版本标签 | 正式发布与可回滚部署 |
