...
- C.3 可额外生成 `sha-<short-sha>` 作为审计标签，但生产部署仍以发布版本标签为准；后续 C.5/C.6 可进一步记录镜像 digest。
- 包可见性、VPS 拉取身份与 GHCR Pull Token 由 C.4 决定；本约定不创建包、不推送镜像，也不授予额外权限。

## 实施边界

本文件只确定稳定的名称与标签语义。C.2 负责生产 Compose 与 Dockerfile，C.3 负责构建/推送工作流，C.4 负责包可见性与拉取授权。
